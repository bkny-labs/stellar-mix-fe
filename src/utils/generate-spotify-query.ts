...
function shuffleArray(array: any[]) {
  ...
  export const getMusicalMood = (data: AppState, ai?: boolean): MusicalMood => {
    let moods: string[] = [];
    ...
    if (ai) {
      ...
      const generalMoods = [data.moodData || 'happy', 'sad', 'angry', 'calm', 'energetic', 'relaxed', 'upbeat'];
...
export const buildPlaylistQuery = (moodData: { moods: string[], genres: string[] }): string => {
  if (!moodData || !moodData.moods || !moodData.genres) {
    console.error('Invalid mood data passed to buildPlaylistQuery:', moodData);
    return '';
  }
